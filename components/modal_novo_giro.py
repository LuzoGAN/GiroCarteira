...
# ========= Layout ========= #
layout = dbc.Modal([
    dbc.ModalHeader(dbc.ModalTitle("Registre um Novo Contato")),
    dbc.ModalBody([
        dbc.Row([
            dbc.Col([
                # Empresa
                ...
                dcc.Dropdown(id='tipo_processo', clearable=False, className='dbc',
                             options=['Telefonema', 'WhatsApp', 'Presencial', 'Email']),
                # Tipo de Processo
                dbc.Label('Ação', html_for='acao'),
                dcc.Dropdown(id='acao', clearable=False, className='dbc',
                             options=['Oferta', 'Empréstimo', 'Consignado', 'Seguro',
                                      'Limite', 'Rural', 'Observação']),
            ], sm=12, md=4),
            dbc.Col([
                dbc.Label("Descrição", html_for='input_desc'),
                dbc.Textarea(id="input_desc", placeholder="Escreva aqui observações sobre o contato...",
                             style={'height': '80%'}),
            ], sm=12, md=8)
        ]),
        html.Hr(),
        dbc.Row([
            dbc.Col([
                dbc.Label("Ofertas", html_for='vara'),
                dbc.RadioItems(id='vara',
                               options=[{'label': 'Crédito', 'value': 'Crédito'},
            ...
            dbc.Col([
                ...
                dbc.RadioItems(id='fase', inline=True,
                               options=[{'label': 'Demonstração', 'value': 'Demonstração'},
                                        {'label': 'Liberação', 'value': 'Liberação'},
            ...
            dbc.Col([
                dbc.Label("Contato", html_for='instancia'),
                dbc.RadioItems(id='instancia',
                               options=[{'label': '1º Contato', 'value': 1},
                                        {'label': 'Outro Contato', 'value': 2}, ])
            ], sm=12, md=3)
        ]),
        html.Hr(),
        dbc.Row([
            dbc.Col([
                ...
                html.Br(),
                dbc.Switch(id='processo_concluido', label="Contato Concluído", value=False),
                dbc.Switch(id='processo_vencido', label="Contato Não Concluído", value=False),
            ...
            dbc.Col([
                ...
                dbc.Row([
                    dbc.Col([
                        dbc.Input(id="input_cliente_cpf", placeholder="CPF do cliente (apenas números)...",
                                  type="number")
                    ])
                ], style={'padding': '15px'}),
            ], sm=12, md=7)
        ...
        dbc.Row([
            dbc.Col([
                dcc.Dropdown(id='input_local_arquivo', clearable=False, className='dbc',
                             placeholder="Local de contato",
                             options=['Agência', 'Reunião', 'Encontro',]),
            ], sm=12, md=5, style={'padding': '15px'}),
            dbc.Col([
                dbc.Input(id="input_no_processo", placeholder="Nº contato", type="number",
                          disabled=False)
...
def crud_processos(n_new, n_save, n_delete, store_int, is_open, store_proc, no_processo, empresa, tipo, acao, vara,
                   fase, instancia, data_inicial, data_final, processo_concluido, processo_vencido, advogados, cliente,
                   cpf_cliente, desc):
    first_call = True if (callback_context.triggered[0]['value'] == None or callback_context.triggered[0][
        'value'] == False) else False
    trigg_id = callback_context.triggered[0]['prop_id'].split('.')[0]
    ...
    if trigg_id == 'save_button_novo_processo':
        ...
        if len(df_int.index) == 0:  # Novo processo
            if None in [no_processo, empresa, tipo, acao, vara, fase, instancia, data_inicial, advogados, cliente,
                        cpf_cliente]:
                return store_proc, ["Todos dados são obrigatórios para registro!"], {'margin-bottom': '15px',
                                                                                     'color': 'red'}, no_processo, empresa, tipo, acao, vara, fase, instancia, data_inicial, data_final, processo_concluido, processo_vencido, advogados, cliente, cpf_cliente, desc, False
            if (no_processo in df_proc['No Processo'].values):
                return store_proc, ["Número de processo ja existe no sistema!"], {'margin-bottom': '15px',
                                                                                  'color': 'red'}, no_processo, empresa, tipo, acao, vara, fase, instancia, data_inicial, data_final, processo_concluido, processo_vencido, advogados, cliente, cpf_cliente, desc, False
            ...
            df_proc.loc[df_proc.shape[0]] = [no_processo, empresa, tipo, acao, vara, fase, instancia, data_inicial,
                                             data_final,
                                             processo_concluido, processo_vencido, advogados, cliente, cpf_cliente,
                                             desc]

            store_proc = df_proc.to_dict()
            no_processo = empresa = tipo = acao = vara = fase = instancia = data_inicial = data_final = advogados = cliente = cpf_cliente = desc = None
            processo_concluido = processo_vencido = False
            return store_proc, ['Processo salvo com sucesso!'], {'margin-bottom': '15px',
                                                                 'color': 'green'}, no_processo, empresa, tipo, acao, vara, fase, instancia, data_inicial, data_final, processo_concluido, processo_vencido, advogados, cliente, cpf_cliente, desc, False
            ...
            df_proc.loc[index, df_proc.columns] = [no_processo, empresa, tipo, acao, vara, fase, instancia,
                                                   data_inicial, data_final, processo_concluido, processo_vencido,
                                                   advogados, cliente, cpf_cliente, desc]
    ...
    if (trigg_id == 'store_intermedio') and is_open:
        try:
            ...
            return store_proc, ['Modo de Edição: Número de Processo não pode ser alterado'], {'margin-bottom': '15px',
                                                                                              'color': 'green'}, no_processo, empresa, tipo, acao, vara, fase, instancia, data_inicial, data_final, processo_concluido, processo_vencido, advogados, cliente, cpf_cliente, desc, disabled
        except:
            no_processo = empresa = tipo = acao = vara = fase = instancia = data_inicial = data_final = advogados = cliente = cpf_cliente = desc = None
            processo_concluido = processo_vencido = False
            return store_proc, [], {}, no_processo, empresa, tipo, acao, vara, fase, instancia, data_inicial, data_final, processo_concluido, processo_vencido, advogados, cliente, cpf_cliente, desc, False
