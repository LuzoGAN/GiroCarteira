...
from app import app
from components import modal_novo_giro, modal_novo_gerente, modal_gerentes
...
def gerar_card_padrao(qnt_procs):
    card_padrao = dbc.Card([
        dbc.CardBody([
            html.H3(f"{qnt_procs} Giros Feitos", style={'font-weight': 'bold', 'color': 'white'})
        ])
            ...
            dbc.Row([
                ...
                dbc.Col([
                    ...
                    dbc.Row([
                        ...
                        dbc.Col([
                            dbc.Button([html.I(className="fa fa-pencil fa-2x")], style={'color': 'black'}, size='sm',
                                       outline=True,
                                       id={'type': 'editar_processo', 'index': int(df_aux['No Processo'])})
                        ], md=2),
                        dbc.Col([
                            dbc.Button([html.I(className="fa fa-trash fa-2x")], style={'color': 'black'}, size='sm',
                                       outline=True,
...
layout = dbc.Container([
    dbc.Row([
        dbc.Col([
            html.H3("ENCONTRE O CLIENTE QUE VOCÊ PROCURA", style={'text-align': 'left', 'margin-left': '32px'})
                ...
                dbc.Col([
                    dbc.Card([
                        dbc.CardBody([
                            dbc.Row([
                                dbc.Col([
                                    html.H3("NOME DO CLIENTE")
                                ], sm=12)
                            ]),
                            dbc.Row([
                                dbc.Col([
                                    dbc.Input(id='processos_filter', placeholder="Insira...", type="name")
                                ], sm=12, md=12, lg=8),
                                dbc.Col([
                            ...
                            dbc.Row([
                                dbc.Col([
                                    dbc.Checklist(
                                        options=[{"label": "1a Contatos", "value": 1},
                                                 {"label": "Outros contatos", "value": 2}],
                                        value=[1, 2],
                                        id="checklist_input"
                                    ),
                                ])
                            ...
                            dbc.Row([
                                dbc.Col([
                                    dbc.Button("Todos os Giros", id="todos_processos", style={'width': '100%'},
                                               color="dark")
                                ])
                            ], style={'margin-top': '24px'})
                        ], style={'margin': '20px'})
...
def generate_cards(n, n_all, adv_filter, proc_button, proc_data, adv_data, switches, checklist, proc_filter, cpf):
    ...
    if (
            trigg_id == '' or trigg_id == 'store_proc' or trigg_id == 'store_adv' or trigg_id == 'todos_processos' or trigg_id == 'switches_input' or trigg_id == 'checklist_input'):
        if trigg_id != 'todos_processos':
            # Filtrando pelos switches
            if (1 and 2) in switches:
                df_proc_aux = df_proc_aux.loc[
                    (df_proc_aux['Processo Concluído'] == 1) & (df_proc_aux['Processo Vencido'] == 1)]
            elif switches == [1]:
                df_proc_aux = df_proc_aux.loc[df_proc_aux['Processo Concluído'] == 1]
            elif switches == [2]:
                df_proc_aux = df_proc_aux.loc[df_proc_aux['Processo Vencido'] == 1]

            if (1 in checklist) and (2 in checklist):
                pass
            elif checklist == [1]:
                df_proc_aux = df_proc_aux.loc[df_proc_aux['Instância'] == 1]
            elif checklist == [2]:
                df_proc_aux = df_proc_aux.loc[df_proc_aux['Instância'] == 2]
    ...
    # Pesquisa de texto por número de processo
        ...
        # Dados
        df_proc_aux = df_proc_aux.loc[df_proc_aux['No Processo'] == proc_filter].sort_values(by='Data Inicial',
                                                                                             ascending=False)
    ...
    # Pesquisa de cliente por CPF
        ...
        if cpf in df_proc['Cpf Cliente'].values:
            ...
            # Dados
            df_proc_aux = df_proc_aux.loc[df_proc_aux['Cpf Cliente'] == cpf].sort_values(by='Data Inicial',
                                                                                         ascending=False)
            ...
            # Iterando sobre os processos
            for i in range(len(df_proc_aux)):
                df_aux, concluido, vencido, color_c, color_v, concluido_text, vencido_text = gerar_icones(df_proc_aux,
                                                                                                          i)
                card = gerar_card_processo(df_aux, color_c, color_v, concluido, vencido, concluido_text, vencido_text)
                cards += [card]
